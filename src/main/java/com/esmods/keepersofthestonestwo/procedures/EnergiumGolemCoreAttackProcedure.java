...
import com.esmods.keepersofthestonestwo.init.PowerModDamageSources;
			...
			if (entity.getPersistentData().getDouble("IA") > 24 && entity.getPersistentData().getDouble("IA") < 26) {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.evoker.cast_spell")), SoundSource.HOSTILE, 1, 1);
					} else {
						...
						for (Entity entityiterator : _entfound) {
							if (!(entityiterator == entity)) {
								if (!(entityiterator instanceof ItemEntity)) {
									entityiterator.hurt(PowerModDamageSources.ENERGIUM_GOLEM_DS, 26);
