	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (((entity.getCapability(PowerModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new PowerModVariables.PlayerVariables())).attack).equals("sand_attack_1")) {
			if ((entity.getCapability(PowerModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new PowerModVariables.PlayerVariables())).power >= 15) {
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.sand.break")), SoundSource.PLAYERS, 1, 1);
					} else {
			...
			if ((entity.getCapability(PowerModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new PowerModVariables.PlayerVariables())).power >= 30) {
					...
					}
					{
						...
						for (Entity entityiterator : _entfound) {
							if (!(entity == entityiterator)) {
								...
								entityiterator.hurt(new EntityDamageSource("power:elemental_powers", entity), 0);
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.sand.fall")), SoundSource.PLAYERS, 1, 1);
					} else {
						...
						}
						{
							...
							for (Entity entityiterator : _entfound) {
								if (!(entityiterator == entity)) {
									...
									entityiterator.hurt(new EntityDamageSource("power:elemental_powers", entity), (float) 5.07);
								}
							}
						}
